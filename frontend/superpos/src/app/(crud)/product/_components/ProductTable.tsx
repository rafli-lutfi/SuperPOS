...
import { truncateName, toIDRCurrency } from "@/utils/formatter";
...
export default function ProductTable() {
    ...
    const handleDeleteButton = async (productId: number, productName: string) => {
        notify.confirm("Are You Sure?", "Delete " + productName).then(async (result) => {
            if (result.isConfirmed) {
                try {
                    await deleter(`${API_URL}/products/${productId}`);
                    await mutate();
                    notify.success("Deleted!", "Product has been deleted");
                } catch (error) {
